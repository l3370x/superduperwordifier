    ...
    socket.emit("check", roomName, function(data) {
      roomExists = data.result;
       if (roomExists) {
          $("#errors").empty();
          $("#errors").show();
          $("#errors").append("Game room <i>" + roomName + "</i> already exists");
        } else {      
...
socket.on("history", function(data) {
  ...
  socket.on("roomList", function(data) {
    $("#rooms").text("");
    $("#rooms").append("<li class=\"list-group-item active\">List of game rooms <span class=\"badge\">"+data.count+"</span></li>");
     if (!jQuery.isEmptyObject(data.rooms)) { 
      ...
      $("#rooms").append("<li class=\"list-group-item\">There are no game rooms yet.</li>");
    }
  });
  ...
  socket.on("inGameConfiguration",function(data){
	var rpWidth = $("#roomsPeople").width();
	if(data == true){
		
		$("#roomsPeople").animate({left:"-="+rpWidth},'slow',
			function(){
				
			});
		$("#chatDiv").animate({left:"-="+rpWidth},'slow',
			function(){
				$("#chatDiv").css({left:0});
				$("#roomsPeople").hide();
			});
